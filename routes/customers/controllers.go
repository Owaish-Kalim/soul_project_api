...
func ListCustomer(w http.ResponseWriter, r *http.Request) ([]Customer, ErrorMessage) {
	...
	("Customer_Email"), ("Customer_Address"), ("Registered_Source") FROM slh_customers WHERE ("Customer_Souls_Id") LIKE  ''||$1||'%' AND 
	("Customer_Name") LIKE ''|| $2 ||'%' AND ("Customer_Gender") LIKE ''|| $3 ||'%' 
	AND ("Customer_Email") LIKE ''|| $4 ||'%' AND ("Registered_Source") LIKE ''|| $5 ||'%' ORDER BY ("CreatedAt") DESC LIMIT $6 OFFSET $7`
